...
"""
import sys
sys.path.append("/Users/ayushgoel/Documents/UniversityOfPennsylvania/Courses/Spring_2023/MEAM-6200/MEAM-6200/Project_1_1/proj1_1/meam620/")
import numpy as np
...
e_, _, _ = Rotation.from_quat(np.array([ 0.69924, 0, 0, -0.714887 ])).as_euler('zxy', degrees=True)
e_ = np.ones(len(x[:, 0]))
for i in range(len(x[:, 0])):
    tmp = state['q'][i,:]
    e, _, _ = quat2euler(tmp)
    # print(Rotation.from_quat([tmp[1],tmp[2],tmp[3],tmp[0]]).as_euler('xyz', degrees=True))
    e_[i] = e

ax = axes
ax.plot(time, e_, time, (np.pi/3)*np.ones(( len( x[:, 0] ), 1) ), 'r.')
ax.legend(('x', 'y', 'z'))
