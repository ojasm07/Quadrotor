...
class SE3Control(object):
    ...
    def update(self, t, state, flat_output):
        ...
        # STUDENT CODE HERE
        kp = np.diag(np.array([6, 6.3, 10]))
        kd = np.diag(np.array([5.2, 5.2, 5]))

        KR = np.diag(np.array([1600, 1650, 20]))
        KW = np.diag(np.array([75, 60, 15]))
        ...
        R_des = Rotation.from_quat(np.array([0.5, 0, 0, 0.8660254])).as_matrix() #for step response in orientation
