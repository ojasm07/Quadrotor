...
from scipy.sparse import lil_matrix as sp
...
class WorldTraj(object):
    ...
    def waypoint_pruning(self, pruned_path):
        ...
        if dmax > 0.5:
            ...
            recResults1 = self.waypoint_pruning(pruned_path[:index+1])
            recResults2 = self.waypoint_pruning(pruned_path[index:])
            x = np.vstack((recResults1[:-1], recResults2))
            return x
    ...
    def __init__(self, world, start, goal):
        ...
        self.time[[0, -1]] *= 2.7
        self.time = np.sqrt(self.time)
        self.time *= np.sqrt(1.65) / self.time

        self.T = np.hstack((0, np.cumsum(self.time)))
    ...
    def update(self, t):
        ...
        if t < t_final: 
            segment_no_of_quadcop = (np.where(self.T -t > 0))[0][0] - 1
            # segment_number = segment_number[0][0] - 1
            current_time = t - self.T[segment_no_of_quadcop]
